...
def permanova_twoway(dm,levels,permutations=200):
    ...
    for i in range(permutations):
        ...
        f_i, f_a, f_b = f_twoway(dm,[(x,y) for x,y in zip(a_levels, [l[1] for l in levels])])
    ...
    for i in range(permutations):
        ...
        f_i, f_a, f_b = f_twoway(dm,[(x,y) for x,y in zip([l[0] for l in levels], b_levels)])
...
def f_twoway(dm, levels):
    ...
    bign = len(list(levels))#number of observations
    dm = np.asarray(dm)#distance matrix
